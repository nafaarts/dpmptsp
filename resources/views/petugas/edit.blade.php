<x-app-layout>
    ...
    <div class="p-6 bg-white border border-gray-200 shadow rounded-lg">
        <form action="{{ route('petugas.update', $user) }}" method="post">
            @csrf
            @method('PUT')

            <div class="mb-3">
                <x-form.label for="nama" class="mb-2" :value="__('Name')" />
                <x-form.input type="text" name="nama" class="w-full" value="{{ old('nama', $user->nama) }}"
                    id="nama" />
            </div>

            <div class="mb-3">
                <x-form.label for="email" class="mb-2" :value="__('Email')" />
                <x-form.input type="email" name="email" class="w-full" value="{{ old('email', $user->email) }}"
                    id="email" />
            </div>
